...
RSpec.describe Book::ISBN::Lookup::OpenLibrary do
  let(:result) { Book::ISBN::Lookup::OpenLibrary.(nil, isbn: isbn) }

  let(:isbn) { Factory::Book.isbn }

  before { NetStub::OpenLibrary.stub_request(isbn, response) }
  ...
  context 'when isbn does not match' do
