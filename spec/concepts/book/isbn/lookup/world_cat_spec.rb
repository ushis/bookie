...
RSpec.describe Book::ISBN::Lookup::WorldCat do
  let(:result) { Book::ISBN::Lookup::WorldCat.(nil, isbn: isbn) }

  let(:isbn) { Factory::Book.isbn }

  before { NetStub::WorldCat.stub_request(isbn, response) }
  ...
  let(:response_authors) { Factory::Book.authors }
  ...
  it 'successfully finds the correct attributes' do
    ...
    expect(result['attributes'][:authors]).to eq(response_authors)
  end
